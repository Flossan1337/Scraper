...
in the live `DATA_DASHBOARD.xlsx` (2026-08-05).** **Anoto added 2026-09-06:** two
new queries/tabs (`ANOTO daily sales RAW`, `ANOTO by product RAW`) off
`anoto_daily_sales_v` (now also carries `restocks`) and the new
`anoto_product_daily_sales_v`, both filtered to `store = 'anoto'` — 24
queries total. (Neo Smart Pen tabs were added the same day and removed again
within the hour: Neo tracking was discontinued, see the script row below.) Both
views re-validated against the xlsx first (0 discrepancies; see
`KNOWN_ISSUES.md` #14 for the inq.shop cents-vs-dollars fix this surfaced).
These were added with Excel COM automation (`Workbook.Queries.Add` + a
`Microsoft.Mashup.OleDb.1` ListObject per query, refreshed with the saved
credentials) rather than by hand — a backup copy with the usual
`DATA DASHBOARD - BACKUP <timestamp>.xlsx` name sits next to the workbook. Batch 4 (the last 9 —
